...
Die konkreten Implementierungen in SQL beziehen sich auf das im Abschnitt \ref{abs:model_mnist} vorgestellte Modell \ref{modell}, welches als laufende Beispielmodell dienen soll.   
\subsection*{Linearkombinationen}
Bei ML-Algorithmen wird oft mit mehrdimensionalen Arrays gearbeitet. Dies ist problemlos mit dem Relationenmodell vereinbar. Das Coordinate-Schema wird einfach mit Attributen, welche die Achen darstellen, erweitert. Für den Kern $K^1 \in \RR^{6 \times 1 \times 5 \times 5}$ ergibt sich die Relation \textbf{K1} zu
