...
\begin{defi}[Quadratische Toeplitz-Matrix]
    ...
    \end{pmatrix}
\end{equation*}
    wird Toeplitz\footnote{Otto Toeplitz 1881-1940}-Matrix genannt. Für die Einträge gilt $A_{i,j}=A_{i+1,j+1}=a_{i-j}$. Für den Fall $a_k=a_{-(n-k)}=a_{k-n}$ für $1 \leq k \leq n-1$ ist $A$ eine zyklische Matrix im Vektor $a=(a_0, \ldots,m a_{n-1})^T$.
\end{defi}

\begin{defi}
    
\end{defi}
Für zweidimensionale Signale kann die Faltung als lineare Transformation mithilfe von Blocktoeplitzmatrizen dargestellt werden.
\begin{defi}
    \label{def:double_circ}
    Eine Matrix $A \in \RR^{n^2 \times n^2}$ heißt doppelt zyklische Blockmatrix, genau dann wenn $A$ eine zyklische Matrix 
\end{defi}
