...
\begin{defi}[Inverse Diskrete Fourier-Transformation]
    Sei $\hat{x}=(\hat{x}_0, \ldots, \hat{x}_{n-1}) \in \mathbb{C}^n$ das Ergebnis der diskreten Fourier-Transformation von $x \in \RR^n$. Mit den Koeffizienten
    \begin{equation*}
...
\end{defi}
Die diskrete Fourier-Transformation aus Definition \ref{def:dft} lässt sich in ein Matrixvektorprodukt $\hat{x}=Fx$ überführen, wobei $F \in \mathbb{C}^{n \times n}$ eine symmetrische Matrix mit der Gestalt
\begin{equation}
    \label{eq:FM}
    F=\begin{pmatrix}
        &1 &1 &1 &\ldots &1 \\
...
\begin{proof}
    ...
    \begin{equation*}
        F^H=\bar{{F}}^T=\bar{F}.
    \end{equation*}
    Mit $W:=F\bar{F}$ gilt $W_{k,j}=\sum_{l=0}^{n-1} \omega^{(j-k)l}$. Ist $k=j$ so ergeben sich die Einträge auf der Hauptdiagonalen von $W$ zu $n$. Ist $k \neq j$, so ist $\omega_0:=\omega^{j-k} \neq 1$ eine $n$-te Einheitswurzel.
    Damit gilt
    \begin{equation*}
...
\end{proof} 
Wird ein zweidimensionales diskretes Signal in Form einer Matrix $X \in \RR^{n}$ betrachtet, lässt sich die zweidimensionale diskrete Fourier-Transformation definieren. 
\begin{defi}
...
\begin{satz}[Zyklisches Faltungstheorem]
    ...
    \begin{equation}
        \mathrm{DFT}(y)=\left(\mathrm{DFT}(f) \odot \mathrm{DFT}(g) \right).
    \end{equation}
...
\begin{itemize}
    ...
    \item[2.] Die Matrix $\hat{Y}= \hat{X} \odot \hat{K}$, welche sich mit der elementweisen Multiplikation ergibt ist zu bestimmen.
    \item[3.] Schließlich stellt $Y=\mathrm{i2DFT}(\hat{Y})$ die Matrixfaltung dar, welche mithilfe der inversen Fourier-Transformation ermittelt wird.    
\end{itemize} 
