...
\begin{defi}[Diskrete Fourier-Transformation]
    \label{def:dft}
    Im Zeitbereich sei ein diskretes Signal $x=(x_0, \ldots, x_{n-1})^T \in \RR^n$ gegeben. Dann wird mit $\hat{x}=(\hat{x}_0, \ldots, \hat{x}_{n-1}) \in \mathbb{C}^n$ das Ergebnis der diskreten Fourier-Transformation, kurz DFT, im Frequenzbereich bezeichnet. Die sogenannten Fourier-Koeffizienten sind als 
    \begin{equation*}
...
\begin{defi}[Inverse Diskrete Fourier-Transformation]
    ...
    \end{equation*}
    lassen sich die Einträge des Vektors $x$ rekonstruieren. Diese Transformation wird inverse diskrete Fourier-Transformation, kurz iDFT, genannt.
\end{defi}
...
\begin{equation*}
    F_{k,j}= \mathrm{e}^{- \frac{2 \pi i}{n} j k}, \; \; 1 \leq k, j \leq n-1
\end{equation*}
...
\begin{defi}
    Sei $X \in \RR^{n \times n}$ gegeben. Die Koeffizienten der zweidimensionalen diskreten Fourier-Transformation, kurz 2-DFT, sind als
    \begin{align*}
    ...
    \end{align*}
    definiert. Mit dieser Darstellung lässt sich die 2-DFT als Matrixmatrixmultiplikation $\hat{X}=FXF^T$ berechnen.
\end{defi}
