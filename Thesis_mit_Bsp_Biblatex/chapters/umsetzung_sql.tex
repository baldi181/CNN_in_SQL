...
\begin{align*}
    ...
    &v \; \; \mathrm{double}),
\end{align*}
wobei $q$ und $p$ die neuen Achsen präsentieren. 
...
\lstinputlisting[label=sql:recursive, caption=Ein Beispiel einer rekursiven Anfrage, language=SQL]{sql_code/recursive_sum_n.txt}
...
Im Hinblick auf die gefaltete Übertragungsfunktion \ref{eq:convlogit} ist eine Summe der Form
\begin{equation}
    \psi \left(\sum_{p=1}^{z_{in}} \alpha_{qp} A_p+ b \mathbf{1} \right)
\end{equation}
zu berechnen. Dabei sind die Matrizen $A_p$ Ergebnisse von Matrixfaltungen, $\alpha_{qp} \in \RR$ beliebige Gewichte, $b$ ein Biasvektor sowie $psi$ eine Aktivierungsfunktion. Zukünftig werden Aktivierungsfunktionen im SQL-Code mit $T$ bezeichnet.
...
TODO beispiel fatten code+Pooling
