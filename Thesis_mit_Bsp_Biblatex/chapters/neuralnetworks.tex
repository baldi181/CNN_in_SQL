...
Wird ein künstliches neuronales Netz als probabilistisches Modell genutzt und sind die Hyperparameter festgelegt, müssen die Modellparameter $\mathcal{W}$ gewählt werden. Um die Approximationsgüte messbar zu machen, werden Fehlerfunktionen genutzt, welche mithilfe des Gradientenverfahrens aus Kapitel \ref{fund} durch die Anpassung der Parameter $\mathcal{W}$ in einem Trainingsprozess minimiert werden.
...
Die Güte eines Modells $f_{Modell}$ wird als Likelihood gegeben einer Trainingsmenge $\mathcal{T}$ gemessen und ist durch 

\begin{equation*}
    L(\mathcal{T},\mathcal{W}):=\prod_{(x,c) \in \mathcal{T}} p_{Modell}(c \; | \; x; \mathcal{W})
\end{equation*}
gegeben\cite{bishop2006pattern}.
Ziel ist es, dass $P_{Modell}$ und $P_{Daten}$ übereinstimmen und daher wird versucht, die Likelihood zu maximieren. Für einen gegebene Trainingsmenge $\mathcal{T}$ soll das Produkt über alle Wahrscheinlichkeiten der korrekten Klassenzugehörigkeiten maximiert werden. Dieser Ansatz wird \textit{Maximum Likelihood-Methode}\cite{ruschendorf2014mathematische} genannt und die Parameterwahl des Modells wird durch eine Lösung des Optimierungsproblems 
\begin{equation}
    \label{opt_likelihood}
...
Für ein Trainingspaar $(x,c) \in \mathcal{T}$ bezeichne $t \in \RR^m$ den Zielvektor mit den Komponenten
\begin{equation*}
    t_i:= \begin{cases}
        1 &, \text{wenn} \, c=c_i \\
        0 &, \text{sonst}
    \end{cases}, \; \; \forall i \in [m].
\end{equation*} 