...
\end{alignat*}
genutzt. Es wird nur eine verdeckte Schicht mit $s_j$ Neuronen betrachtet. Die Verallgemeinerung für beliebig viele Schichten ist analog. Die mehrmalige Anwendung der Kettenregel \ref{s:chainrule} auf die Fehlerfunktion (\ref{eq:MSE_single}) liefert
...
\begin{equation*}
    \delta_k:= e_k \psi'(V_k)
\end{equation*}
als sogenannter lokaler Fehler.
...
\begin{equation*}
    \Delta b_k^l = -\lambda \, \frac{\partial E_x}{\partial b_{k}^l} 
                 = -\lambda \, \delta_k.
\end{equation*}
Die Gradienten für die Parameter der verdeckten Schicht lassen sich mithilfe der lokalen Fehler der darüberliegenden Ausgabeschicht berechnen, also
\begin{align*}
...
\begin{equation*}
    \Delta b_j^h = -\lambda \, \frac{\partial E_x}{\partial b_{j}^h} 
                 = -\lambda \, \delta_j.
\end{equation*}
Die Backpropagation wird als iterativer Algorithmus beschrieben. Daher tauchen im Algorithmus \ref{alg:online_backprop} Variabeln in Abhängigkeit von $n$ auf. Als Abbruchbedingung kann eine maximale Anzahl von Iterationen vorgegeben werden. Andere Abbruchbedingungen können mit der Norm der Abstiegsrichtungen\cite{bishop2006pattern} oder abhängig vom Traingsfehler formuliert werden.
