...
\end{equation*}
zur Verfügung. Für ein Trainingspaar $\left(x,f(x)\right) \in \mathcal{T}$ bezeichne $t(x,c) \in \RR^s$ den Zielvektor der Klasse $c$ mit sogenannter (1 aus s)-Kodierung. Die Komponenten des Zielvektors sind
\begin{equation*}
    t_k(x,c):= \begin{cases}
        1 &, \text{wenn} \; k=c \\
        0 &, \text{sonst}
...
\end{equation} 
gelöst werden soll. Dabei sei bemerkt, dass dies unabhängig von den Hyperparametern geschieht. Im Folgenden wird der Backpropagationsalgorithmus als ein Lösungsansatz der Aufgabe (\ref{eq:Emin}) vorgestellt.  
...
Die Optimierung der Parameter geschieht iterativ, dargestellt mit dem Index $n$, und das Gradientenverfahren besteht jeweils aus zwei Schritten. Zuerst wird eine Abstiegsrichtung 
\begin{equation}
    \label{eq:gradE}
...
Der Online-Backpropagationsalgorithmus, in der Literatur auch als \textit{Stochastic Gradient Descent} bezeichnet, wird im Folgenden erläutert. Grundsätzlich lässt sich das Verfahren in zwei Schritte einteilen.
