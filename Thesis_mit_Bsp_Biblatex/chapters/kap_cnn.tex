...
\end{equation*}
Um $ \Delta K^2_{q,p}$ zu bestimmen, ist zunächst der lokale Fehler der darüberliegenden Flatten-Schicht zu ermitteln. Der lokale Fehler $\delta^f \in \RR^{192}$ in der Flatten-Schicht wird mithilfe des lokalen Fehlers des FNN berechnet und lautet
\begin{align*}
...
\end{align*}
Es gilt $\delta^{C^2} \in \RR^{8 \times 8 \times 12}$. Nun kann $\Delta K^2_{q,p}$ an einer Stelle $(u,v)$ berechnet werden.
\begin{align*}
...
\end{align*} der lokale Fehler $\delta^{C^2}_{q,\psi}$ der Faltungschicht $C^2$ vor der Anwendung der logistischen Funktion und
\begin{equation*}
    P^1_{p,rot180}(u-i,v-j):=P^1_p(i-u,j-v).
\end{equation*}
Dann gilt
\begin{align*}
    \Delta K^2_{q,p}(u,v) &= - \lambda \, \sum_{i=1}^8 \sum_{j=1}^8 P^1_{p,rot180}(u-i,v-j) \cdot \delta^{C^2}_{q,\psi}(i,j) \\
     \Rightarrow \Delta K_{q,p}^2 &=- \lambda \, P^1_{p,rot180} \ast \delta^{C^2}_{q,\psi} , \; \; \forall q \in [12], \forall p \in [6].
\end{align*}
Dies ist besonders für die effiziente Implementierung der Rückwärtsrechnung nützlich, da Abstiegsrichtungen wieder mithilfe von Faltungsoperationen berechnet werden können. Analog ergibt sich 
\begin{equation*}
...
\begin{align*}
    \delta^{S^1}_{p}(i,j) &= \frac{\partial E}{\partial S^1_{p}(i,j)} \\
    &= \sum_{q=1}^{12} \sum_{u=-2}^2 \sum_{v=-2}^2 \frac{\partial E}{\partial C^2_{q,\psi}(i+u,j+v)} \cdot \frac{\partial C^2_{q,\psi}(i+u,j+v)}{\partial S_{p}^1(i,j)} \\
    &= \sum_{q=1}^{12} \sum_{u=-2}^2 \sum_{v=-2}^2 \delta^{C^2}_{q,\psi}(i+u,j+v) \cdot \frac{\partial}{\partial S_p^1(i,j)}  \left( \sum_{p=1}^6 \sum_{u=-2}^2 \sum_{v=-2}^2 P^1_p(i,j) \cdot K^2_{q,p}(u,v) +b_q^2\right) \\
    &= \sum_{q=1}^{12} \sum_{v=-2}^2 \sum_{u=-2}^2 \delta_{q,\psi}^{C^2}(i+u,j+v) \cdot K^2_{q,p}(u,v), \; \; \forall i,j \in [12], \forall p \in [6]
\end{align*}
Mit $K^2_{q,p,rot180}(-u,-v):=K^2_{q,p}(u,v)$ ist
\begin{align*}
    \delta^{S^1}_{p}(i,j)&=\sum_{q=1}^{12} \sum_{v=-2}^2 \sum_{u=-2}^2 \delta_{q,\psi}^{C^2}(i-(-u),j-(-v)) \cdot K^2_{q,p,rot180}(-u,-v) \\
    \Rightarrow \delta^{S^1}_{p} &= \sum_{q=1}^{12} \delta_{q, \psi}^2 \ast K_{q,p,rot180}^2, \; \; \forall p \in [6]
    \end{align*}
Es gilt  $\delta^{S^1} \in \RR^{12 \times 12 \times 6}$. Mit dem Upsampling
\begin{equation*}
    \delta^{C^1}_p= \delta^{S^1}_p \left( \lceil i/2 \rceil, \lceil j/2 \rceil \right), \forall i,j \in [24], \forall p \in [6]
\end{equation*}
kann nun der Gradient $\Delta K^1_{p,1}$ an einer Stelle $(u,v)$ mit
\begin{align*}
...
\begin{align*}
    \Delta K^1_{p,1}(u,v) &= - \lambda \, \sum_{i=1}^{24} \sum_{j=1}^{24} X_{rot180}(u-i,v-j) \cdot \delta^{C^1}_{p,\psi}(i,j) \\
     \Rightarrow \Delta K_{p,1}^1 &=- \lambda \, X_{rot180} \ast \delta^{C^2}_{q,\psi} , \; \; \forall p \in [6].
\end{align*}
