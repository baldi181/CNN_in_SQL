...
\begin{alignat*}{2}
    K^{1}_{1,p}(u,v) &\sim  \mathcal{N} \left(0, \frac{2}{5 \cdot 5 \cdot (1+6)}\right), \; \; &&1 \leq p \leq 6,\\
    K^{2}_{p,q}(u,v) &\sim  \mathcal{N} \left(0, \frac{2}{5 \cdot 5 \cdot (6+12)}\right),\; \; &&1 \leq p \leq 6, 1 \leq q \leq 12,\\
    W(i,j) &\sim \mathcal{N} \left(0, \frac{2}{192+10}\right).
\end{alignat*}
Die Parameteranzahl ist damit 
\begin{equation*}
    \underbrace{(5 \cdot 5 +1) \cdot 6}_{\text{Gewichte in $C^1$}}+\underbrace{(5 \cdot 5 \cdot 6+1) \cdot 12}_{\text{Gewichte in $C^2$}}+\underbrace{ 10\cdot (192+1)}_{\text{Gewichte im FFN}}=3898.
\end{equation*}
...
\begin{align}
    \label{eq:C1_forw}
    \begin{split}
    C_p^1 &= \psi(X \ast K_{1,p}^1+b_p^1 \mathbf{1}) \\
    C_p^1(i,j)&=\psi \left(\sum_{u=0}^4 \sum_{v=0}^4 X(i+u,j+v) \cdot K_{1,p}^1(u,v) +b_p^1\right)
    \end{split}
\end{align}
...
\begin{align}
    \label{eq:C2_forw}
    \begin{split}
        C_q^2 &= \psi \left( \sum_{p=1}^6 P_p^1 \ast K_{p,q}^2 + \mathbf{1} b_q^2\right) \\
        C_q^2(i,j) &= \psi \left( \sum_{p=1}^6 \sum_{u=0}^4 \sum_{v=0}^4 P^1_p(i+u,j+v) \cdot K_{p,q}(u,v) +b_q^2\right)
    \end{split}
\end{align}
...
\end{equation}
für $1 \leq q \leq 12$. Es gilt $P^2_q \in \RR^{4 \times 4 \times 1}$. Jede dieser $4 \times 4$- Matrizen wird durch die Flatten-Funktion $T_f$ aus Definition \ref{def:flatten} in einen Vektor $f_q \in \RR^{4 \cdot 4 \cdot 1}$ umgewandelt. Anschließend werden diese Vektoren zu einem Vektor zusammengefasst, indem sie einfach untereinander geschrieben werden. Dies sei durch
\begin{equation}
    \label{eq:f_forw}
    f=T_f\left(\{P^2_q\}_{q=1, \ldots, 12}\right)
\end{equation}
beschrieben. 
Die Umkehrung wird mit
\begin{equation}
    \label{eq_f_ford_inv}
    \{P^2_q\}_{q=1, \ldots, 12}=T_f^{-1}(f).
\end{equation}
bezeichnet.
...
\begin{equation}
    \label{eq:E_forw}
    E=\frac{1}{2} \sum_{i=1}^{10} \left(y(i)-t(i)\right)^2,
\end{equation}
