...
\begin{align}
    \begin{split}
        \Delta W(k,j) &= -\lambda \, \frac{\partial E}{\partial W(k,j)}\\
                       &= -\lambda \, \frac{\partial L}{\partial y(k)} \cdot \frac{\partial y(k)}{\partial W(k,j)}\\
                       &=-\lambda \, (t(k)-y(k)) \cdot \frac{\partial}{\partial W(k,j)} \psi \left(\sum_{j=1}^{192} W(k,j) f(j)+b(k)\right)\\
                       &=-\lambda \, (t(k)-y(k)) \cdot y(k)(1-y(k)) \cdot f(j).
     \end{split}
\end{align}
Mit $\delta(k)=\left((t(k)-y(k)) \cdot y(k)(1-y(k))\right)$ für $1 \leq k \leq 10$ lassen sich die Abstiegsrichtungen als dyadisches Produkt  
\begin{align*}
    \Delta W(k,j)&=-\lambda \, \left(\delta(k) \cdot f(j) \right) \\
    \Rightarrow  \Delta W&=-\lambda \,\left(\delta \otimes f^T \right)   
\end{align*}
darstellen. Analog gilt
\begin{equation}
\Delta b= -\lambda \delta
\end{equation}
